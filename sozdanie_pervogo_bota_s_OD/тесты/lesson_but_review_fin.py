...
async def start(update, context):
    ...
    await update.message.reply_text(
        "Нажми на кнопку ниже, чтобы начать распознавание.",
...
*Если не обозначить никакой класс, то модель найдёт все.''',
        ...
        await query.answer(
            f"Установлено значение метрики 'пересечения на объединение' (iou) на {iou}"
            )

    elif data == "proceed":
        if not context.user_data.get("parameters"):
            await query.answer("Пожалуйста, выбери параметры распознавания.")
            return

        context.user_data["proceed"] = True
        await query.answer("Параметры приняты. Теперь отправь фото для распознавания.")
...
async def detection(update, context):
    ...
    if update.message and (update.message.photo or update.message.document):
        if update.message.document and update.message.document.mime_type not in [
            ...
            await update.message.reply_text(
                "Пожалуйста, отправь изображение в формате JPEG или PNG."
                                            )
            return
